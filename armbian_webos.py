#!/usr/bin/env python3
"""
Armbian WebOS Terminal — симуляция ОС на Python с реальным OTA‑обновлением.
Версия 1.0.0
"""
...
# Замените на реальный репозиторий для настоящего OTA
...
cwd = ["/", "home", "armbian"]  # текущий путь в виде списка

# ─── Вспомогательные функции для работы с виртуальной ФС ────────────
...
def ensure_dir(node, name):
    """Создать подпапку в узле, если её нет."""
...
def cmd_man(args):
    manuals = {
        ...
        "apt": "apt - менеджер пакетов (симуляция).",
...
def cmd_ls(args):
    ...
    if long_format:
        ...
        for name, n in entries:
            if n["type"] == "dir":
                line.append(f"\033[94m{name}\033[0m")
            else:
                line.append(name)
...
def cmd_history(args):
    for i, entry in enumerate(readline.get_history_item(i+1) for i in range(readline.get_current_history_length())):
        if entry:
            print(f"{i+1}  {entry}")
...
# ─── Пакетный менеджер (симуляция) ─────────────────────────────────
def cmd_apt(args):
    if not args:
        print("apt: update | upgrade | install")
        return
    sub = args[0]
    if sub == "update":
        print("Сущ:1 http://deb.armbian.com bookworm InRelease")
        print("Получено 45.2 kB за 2с")
        print("Чтение списков пакетов… Готово")
    elif sub == "upgrade":
        print("Чтение списков пакетов… Готово")
        print("Построение дерева зависимостей…")
        print("Будут обновлены: base-files armbian-firmware")
        print("Загрузка…")
        print("Настройка пакетов…")
    elif sub == "install":
        pkg = args[1] if len(args) > 1 else None
        if pkg:
            print(f"Установка {pkg}... Готово.")
        else:
            print("apt install: укажите пакет")
    else:
        print(f"apt: неизвестная команда '{sub}'")
...
def cmd_ota_update(args):
    ...
    # 4. Сохраняем текущий скрипт как резервную копию
    current_file = Path(__file__).resolve()
    backup_file = current_file.with_suffix(".py.bak")
    shutil.copy2(current_file, backup_file)

    # 5. Записываем новую версию
    try:
